...
public class EndListener implements Listener {
    ...
    public void onEntityDamage(EntityDamageEvent event) {
        if (event.getEntity() instanceof EnderDragon && event.getEntity().getWorld().getEnvironment() == Environment.THE_END) {
    ...
    public void onPortal(PlayerPortalEvent event) {
        ...
        if (event.getTo().getWorld().getEnvironment() == Environment.NORMAL) { // Leaving the End
        ...
        } else if (event.getTo().getWorld().getEnvironment() == Environment.THE_END) { // Entering the end
...
//            }
    ...
    public static void saveEndReturn() {
        Bukkit.getScheduler().scheduleAsyncDelayedTask(Foxtrot.getInstance(), () -> Foxtrot.getInstance().runRedisCommand(redis -> {
            redis.set("endReturn", Foxtrot.PLAIN_GSON.toJson(endReturn));
            return null;
        }));
    }
    ...
    public static void loadEndReturn() {
        ...
        Foxtrot.getInstance().runRedisCommand(redis -> {
            if (redis.exists("endReturn")) {
                endReturn = Foxtrot.PLAIN_GSON.fromJson(redis.get("endReturn"), Location.class);
            } else {
