...
@CommandAlias("addbalance|addbal")
...
public class AddBalanceCommand extends BaseCommand {
    ...
    public static void setBal(Player sender, Player player, Float amount) {
        if (amount > 10000 && sender != null && !sender.isOp()) {
        ...
        Player targetPlayer = Foxtrot.getInstance().getServer().getPlayer(player.getUniqueId());
        ...
        Foxtrot.getInstance().getWrappedBalanceMap().setBalance(player.getUniqueId(), amount);
    }
